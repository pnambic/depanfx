...
For many analysis efforts, this is too coarse of a granularity
to extract any insight from the node view.
There are often too many edges to see any interesting interactions.

The `Edge Visibility` > `Edge Filters` context menu item provides an
additional edge filtering behavior separate from the rendering based
grouping provided by other tools.

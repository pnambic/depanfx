...
public class DepanFxNodeFiltersSequenceItem extends DepanFxNodeFiltersTableItem {

  private boolean childrenLoaded = false;

  private DepanFxNodeFiltersDialogRegistry nodeFiltersDialogRegistry;
  ...
  private ObservableList<TreeItem<DepanFxNodeFiltersTableMember>> buildChildren() {
    ...
    DepanFxNodeFiltersSequenceMember seqMember =
        (DepanFxNodeFiltersSequenceMember) getValue();

    ObservableList<TreeItem<DepanFxNodeFiltersTableMember>> result =
        FXCollections.observableArrayList();

    seqMember.streamMembers()
        .map(f -> buildTableItem((DepanFxBaseFilterData) f))
        .forEach(result::add);
    return result;
  }
